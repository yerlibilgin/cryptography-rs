...
use {
    ...
    bcder::{
        decode::{BytesSource, Constructed, DecodeError, IntoSource, Source}, encode, encode::{PrimitiveContent, Values}, BitString, Captured, Integer, Mode,
        OctetString,
        Oid,
        Tag,
    },
...
impl AlgorithmIdentifier {
  ...
  fn encoded_values(&self, mode: Mode) -> impl Values + '_ {
    // RFC 5758: For ECDSA with SHA-2, parameters MUST be omitted.
    // OID prefix 1.2.840.10045.4.3 corresponds to ecdsa-with-SHA2 family
    let ecdsa_sha2_prefix = [0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x04, 0x03];
    let is_ecdsa_sha2 = self.algorithm.0.as_ref().starts_with(&ecdsa_sha2_prefix);

    let params = if is_ecdsa_sha2 {
      None
...
impl Debug for Certificate {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    ...
    s.field(
      "signature",
      &format_args!("{} (unused {})", hex::encode(self.signature.octet_bytes()), self.signature.unused()),
    );
    s.finish()
  }
}
...
impl Certificate {
  ...
  pub fn iter_extensions(&self) -> impl Iterator<Item = &Extension> {
    self.tbs_certificate.extensions.iter().flat_map(|x| x.iter())
  }
}
...
impl TbsCertificate {
  ...
  pub fn encode_ref(&self) -> impl Values + '_ {
    encode::sequence((
      ...
      self.issuer_unique_id.as_ref().map(|id| id.encode_ref_as(Tag::CTX_1)),
      self.subject_unique_id.as_ref().map(|id| id.encode_ref_as(Tag::CTX_2)),
      self
        .extensions
        .as_ref()
...
impl Extension {
  ...
  pub fn encode_ref(&self) -> impl Values + '_ {
    encode::sequence((
      self.id.encode_ref(),
      if self.critical == Some(true) { Some(true.encode()) } else { None },
